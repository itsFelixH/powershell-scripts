...
<#
.SYNOPSIS
    Batch converts video files to H.265/HEVC MKV format.
.DESCRIPTION
    Scans a source directory for common video formats and compresses them using the 
    libx265 codec (10-bit). Optimized for high-quality archival. Includes a 
    batch-pause feature to prevent long-running unattended processes from overheating 
    hardware or to allow periodic review.
.PARAMETER Path
...
Write-Host "--- CONVERT TO HEVC ---" -ForegroundColor Cyan
Write-Host "-----------------------`n"
...
if ($numFiles -eq 0) {
    Write-Host "No video files found in '$resolvedSource'" -ForegroundColor Yellow
    return
}
...
foreach ($file in $videoFiles) {
    $numConverted++
    $outputFile = Join-Path $resolvedDest "$($file.BaseName)_x265.mkv"

    Write-Host "[$numConverted/$numFiles] Processing: $($file.Name)" -ForegroundColor White
    Write-Host "  Original Size: $(Format-FileSize -size $file.Length)" -ForegroundColor DarkGray

    if ($PSCmdlet.ShouldProcess($file.Name, "Convert to HEVC MKV")) {
        ...
        $elapsed = Measure-Command {
            # Standard High Quality 10-bit HEVC parameters
            $ffmpegArgs = @(
                "-v", "quiet", "-stats", "-y", "-hide_banner",
                "-i", $file.FullName,
                "-c:v", "libx265", "-pix_fmt", "yuv420p10le", "-profile:v", "main10",
                "-c:a", "copy", # Copy audio to preserve quality by default
                $outputFile
            )
            & ffmpeg @ffmpegArgs
        }
